...
async function fetchTickets() {
  try {
    ...
    const response = await axios.get(`/ticket?cursor=${ticketCursor || ""}&filter=${filter || ""}`);
      ...
      //   console.log(ticket);
        ...
        ticketItem.innerHTML = `<div class="flex itmes-center justify-between border-b-2 border-white">
          ...
          <div class="text-black text-sm my-3 font-bold"> Conversation</div>
            <p class="text-black my-3">${ticket.conversation[ticket.conversation.length - 1].role}: <span class="text-gray-700">${ticket.conversation[ticket.conversation.length - 1].body}</span></p>
          </div>
          `;
